...
// Validasi input, memastikan tidak ada kolom yang kosong
if (id_barang.isEmpty() || nama_barang.isEmpty() || harga_jual.isEmpty() || harga_beli.isEmpty() || stok.isEmpty()) {
    JOptionPane.showMessageDialog(this, "Semua kolom wajib diisi!", "Validasi", JOptionPane.ERROR_MESSAGE);
    return;
}
...
try {
    conn = Koneksi.getConnection();  // Pastikan koneksi berhasil
    String sql = "UPDATE barang SET nama_barang = ?, harga_jual = ?, harga_beli = ?, stok = ?, satuan = ? WHERE id_barang = ?";
    st = conn.prepareStatement(sql);

    st.setString(1, nama_barang);
    st.setString(2, harga_jual);
    st.setString(3, harga_beli);
    st.setString(4, stok);
